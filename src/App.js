...
export default function CardDetails() {
  ...
  const handleCountryChange = (e) => {
    setAdress({ ...address, country: e.target.value });
  };

  const handlePostalCodeChange = (e) => {
    setAdress({ ...address, postalCode: e.target.value });
  };

  const handleAddressUpdate = (e) => {
    setCard({ ...card, ...address });
  };
  return (
    <div className="py-4 max-w-sm mx-auto">
      ...
      <fieldset className="mt-6 bg-white">
        ...
        <button
          ...
          className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 mt-10"
        >
          Adresi Güncelle
        </button>
      </fieldset>
